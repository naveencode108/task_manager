...
import { deleteTask, getAlltask, markImportantTask } from "../../../services/actions/taskApi";
  ...
  return (
    <div className="p-1">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 overflow-y-auto">
        {alltask?.filter((item) => item.important == true).length > 0 ? (
          alltask.map(
            (tasks, index) =>
              tasks?.important && (
